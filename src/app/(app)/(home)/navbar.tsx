...
const NavbarItem = ({ href, children, isActive }: NavbarItemProps) => {
    return (
        <Button asChild variant={'outline'} className={cn("hover:bg-transparent rounded-full border-transparent hover:border-primary px-3.5  text-lg",
    ...
    return (
        <nav className="h-20 flex justify-between border-b font-medium border-b-black dark:border-b-slate-700 bg-white dark:bg-slate-900">
            <Link href={'/'} className="pl-6 flex items-center">
                <span className={cn(poppins.className, "text-4xl font-semibold text-slate-900 dark:text-slate-100")}>
                    Payload
                </span>
            </Link>

            <NavbarSidebar open={isSidebarOpen} onOpenChange={setSidebarOpen} items={navbarItems} />
            ...
            <div className="lg:hidden flex ml-3 items-center justify-center">
                <Button asChild variant={'ghost'} onClick={() => setSidebarOpen(true)} className="size-12 border-transparent bg-white">
                    <MenuIcon />
                </Button>
            </div>
        </nav>
